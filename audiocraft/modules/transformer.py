...
class StreamingMultiheadAttention(StreamingModule):
    ...
    def forward(self, query, key, value, key_padding_mask=None, need_weights=False, attn_mask=None,
        ...
        if self.custom_attention:
            print("Query memory address:", id(query))
            print("Key memory address:", id(key))
            context, attention_weights = self.attn(query, key, value, mask=attn_mask)
        else:
            context, attention_weights = self.attn(query, key, value, key_padding_mask=key_padding_mask,
